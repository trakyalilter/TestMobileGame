...
- **⚠ Tooling note:** UI scripts don't fully compile in headless `--check-only`/boot — identifier/scope slips surface only at runtime (hit `existing` + `sm` this session). Verify UI edits by reading; re-extract Godot to `$TEMP/godot_check/` from the Downloads zip when available.
