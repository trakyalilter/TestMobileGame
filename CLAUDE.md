...
- **⚠ Tooling note:** `--check-only --script X.gd --path <proj>` is SYNTAX-only — it resolves autoloads but MISSES undeclared locals / type errors (this is how `existing` + `sm` slipped this session). The reliable check is a **FULL HEADLESS BOOT**: `Godot_console.exe --headless --quit-after 18 --path <proj>` then grep `SCRIPT ERROR|not declared|Nonexistent function|Cannot infer`. The boot DOES compile UI tile scripts (module_card etc.) because `main._init_pages` → `designer_page._ready` → `rebuild_storage` instantiates them. Do NOT use a `SceneTree --script` force-loader — it bypasses autoload registration and floods false "Identifier not found: GameState/UITheme" errors. Godot binary: extract `~/Downloads/Godot_v4.5.1-stable_win64.exe.zip` to `$TEMP/godot_check/`.
...
2. **NG+ Loop 1 (Corrosion, Z12–Z15) is complete, and as of v174 it has a full gear ladder** — every sector Z11–Z15 has its own weapon and defence tier, and `boss_gearcheck` reports 15/15 honouring the gear rule at 21 trials. **Owner ruling 2026-08-11: slow-farming zone N+1 trash with maxed zone-N gear is an INTENDED Melvor-style bootstrap — leave it.** `zone_gate_check` already encodes that: those cells report `BOOT` and PASS (3 of them today, Z2→Z3, Z4→Z5, Z5→Z6). The description here previously called all its failures "slow-farm" cells, which was wrong and is what the ruling was made against. Its 5 remaining failures are two DIFFERENT problems, both still open: **1 BLOCK** (Z3→Z4 — a clean Zone-4 COMMON set *dies* to z4_glacial_drone, so the intended answer cannot do the job, violating Rule B) and **4 NOGAIN** (Z6→Z7 11 vs 12, Z7→Z8 16 vs 16, Z8→Z9 13 vs 13, Z9→Z10 18 vs 17 kills per 180s — the next gear tier is not an upgrade, and in three of them maxed old gear is equal or FASTER than the new tier's commons: a dominated choice). Also still open: **DECISION #30** (NG+ loop boundary — Fleet Siege Gate vs clear+Warp). Then **Loop 2 — Plasma frontier Z16–Z19** (#28).
