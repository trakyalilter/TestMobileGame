...
# ── P3 Boss-trait per-fight state (v139d) — reset in spawn_enemy ──
var _ra_steps := 0            # reactive_armor: DEF-mult steps applied
var _ra_hits := 0             # reactive_armor: landed player hits since last step
var _ag_added := {}           # adaptive_grid: raw resist added per channel
var _cn_count := 0            # charge_nuke: enemy swings since last nuke
var _cn_is_nuke := false      # charge_nuke: this swing is the nuke
var _sus_timer := 0.0         # sustain-pulse accumulator
...
# v149 UNIFORM ATTACK CADENCE (owner rule, 2026-07-26):
#   "one base attack interval everywhere — all weapons, all enemies. If a bonus
#    activates or an enemy skill fires it speeds up, but at DEFAULT they are the
#    same."
# Every authored atk_interval in enemy_db and in shipyard_manager.modules is this
# value. Variation is allowed ONLY from bonuses: the reciprocal rarity roll
# (shipyard_manager.generate_module_drop), affixes (servo_overclock), set
# bonuses, atk_speed_mult, enrage, jamming, and boss skills. Never from the
# authored base.
#
# WHY 2.0: it leaves 40 of 59 weapon modules untouched (every kinetic/energy/cryo
# gun already fired at 2.0), so every per-SHOT mechanic keyed to them — ammo burn,
# hull/shield_heal_on_hit, reactive_armor stepping, adaptive_grid ramp,
# static_burst, vuln uptime — is bit-identical. It also gives exactly 5 enemy
# swings per consumable_cooldown_max (10.0s) window, so the reactive auto-consume
# threshold is always crossed with swings left to answer, and it is the SMALLEST
# viable default, which minimises the one-shot spike risk of charge_nuke.
#
# INVARIANT: the deepest player stacking path is rarity-roll (x0.6) x
# MAX_ATK_SPEED_MULT (3.0) = base x0.20. DEFAULT_ATTACK_INTERVAL * 0.20 must stay
# ABOVE MIN_ATTACK_INTERVAL or the clamp starts binding and DPS silently caps.
# 2.0 * 0.20 = 0.40 > 0.30. Do not lower this below 1.5 without raising the floor.
...
# v139d Rare gate (owner rule, 2026-07-17): sub-Rare gear must NOT beat
# bosses — including well-rolled Uncommon — but the combat math stays PURE
# (a hard x0.25 sub-Rare damage floor was tried and REJECTED: "do not break
# the math"). The separation lives in the ITEM numbers instead: rarity stat
# ranges are disjoint (Uncommon caps x1.20, Rare floors x1.25) and Uncommon
...
const TRINITY_SET_BONUSES = {
	...
	# dead (see the hit-chance removal note in do_player_attack), which made this
	# the weakest 3-piece in the game. Re-pointed at the SENSOR/loot axis, which
...
var zones = {
	...
	"mars_debris": {
		...
		"difficulty": 3,
		"enemies": ["z3_scavenger_mech", "z3_martian_sentry", "z3_derelict_frigate", "z3_boss_warmaster"],
	...
	"cryofield": {
		...
		"difficulty": 4,
		"enemies": ["z4_ice_wraith", "z4_cryo_sentinel", "z4_glacial_drone", "z4_boss_overseer"],
	...
	"sector_alpha": {
		...
		"difficulty": 5,
		"enemies": ["z5_xenon_scout", "z5_xenon_corvette", "z5_alien_frigate", "z5_boss_harbinger"],
	...
	"sector_beta": {
		...
		"difficulty": 6,
		"enemies": ["z6_defense_turret", "z6_mining_golem", "z6_ore_guardian", "z6_boss_colossus"],
	...
	"sector_gamma": {
		...
		"difficulty": 7,
		"enemies": ["z7_shard_swarm", "z7_energy_wraith", "z7_gamma_beast", "z7_boss_sovereign"],
	...
	"sector_delta": {
		...
		"difficulty": 8,
		"enemies": ["z8_prism_drone", "z8_crystal_golem", "z8_nebula_phantom", "z8_boss_warden"],
	...
	"sector_zeta": {
		...
		"difficulty": 9,
		"enemies": ["z9_plague_drone", "z9_bio_horror", "z9_quarantine_mech", "z9_boss_patient_zero"],
	...
	"sector_epsilon": {
		...
		"difficulty": 10,
		"enemies": ["z10_void_stalker", "z10_temporal_phantom", "z10_primordial_titan", "z10_boss_leviathan"],
	...
	"the_rift": {
		...
		"difficulty": 12,
		"enemies": ["z12_acid_revenant", "z12_rust_horror", "z12_caustic_leviathan", "z12_boss_rift_warden"],
	...
	"the_verdigris": {
		...
		"difficulty": 13,
		"enemies": ["z13_blight_drone", "z13_corroded_golem", "z13_patina_phantom", "z13_boss_verdigris_warden"],
	...
	"the_dissolution": {
		...
		"difficulty": 14,
		"enemies": ["z14_dissolution_wraith", "z14_caustic_golem", "z14_toxin_sentinel", "z14_boss_dissolution_tyrant"],
	...
	"the_caustic_core": {
		...
		"difficulty": 15,
		"enemies": ["z15_caustic_revenant", "z15_meltdown_colossus", "z15_blight_titan", "z15_boss_caustic_sovereign"],
...
var enemy_db = {
	...
	"z3_scavenger_mech": {
		"name": "Scavenger Mech",
		"stats": {"hp": 675, "atk": 32, "def": 12, "atk_interval": 2.0, "accuracy": 35},
		...
		"xp": 50, "zone": 3, "resist_k": 0.37, "resist_e": 0.00, "resist_x": -0.30, "dmg_type": "explosive"
	},
	"z3_martian_sentry": {
		"name": "Martian Sentry",
		"stats": {"hp": 864, "max_shield": 300, "atk": 47, "def": 15, "atk_interval": 2.0, "accuracy": 40},
		...
		"module_drop_pool": ["z3_kinetic", "z3_energy", "z3_missile", "z3_shield", "z3_armor", "z3_battery", "z3_sensor"],
		"xp": 60, "zone": 3, "resist_k": -0.30, "resist_e": 0.37, "resist_x": 0.00, "dmg_type": "explosive"
	},
	"z3_derelict_frigate": {
		...
		"stats": {"hp": 1240, "max_shield": 248, "atk": 41, "def": 18, "atk_interval": 2.0, "accuracy": 42},
	...
	"z4_ice_wraith": {
		...
		"xp": 120, "zone": 4, "resist_k": 0.37, "resist_e": -0.30, "resist_x": 0.00, "dmg_type": "energy"
	},
	"z4_cryo_sentinel": {
		"name": "Frost Sentinel",
		...
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z4_kinetic", "z4_energy", "z4_missile", "z4_shield", "z4_armor", "z4_battery", "z4_sensor"],
		"xp": 140, "zone": 4, "resist_k": 0.00, "resist_e": 0.37, "resist_x": -0.30, "dmg_type": "energy"
	},
	"z4_glacial_drone": {
		"name": "Glacial Drone",
		...
		# This one was 7 swings @ 1.8s = 12.6s, re-derived to 6 @ 2.0s = 12.0s. Every
		# nuke period stays ABOVE consumable_cooldown_max (10.0s), so the shield kit is
		# guaranteed off cooldown when the telegraph fires.
		#
		# `mult` HAD TO MOVE WITH IT. The nuke REPLACES a swing for atk x mult, so the
		# damage over one period is atk x (every_n - 1 + mult). Rescaling atk to hold
		# dps while changing every_n does NOT hold that sum — leaving mult alone moved
		# nuke-inclusive dps by up to +8% and (measured) killed tier-matched Common at
		# Z5 e4, breaking the idle rule. The DPS-preserving re-derivation is
		#     new_mult = 1 + (old_mult - 1) x new_every_n / old_every_n
		# which is independent of atk and interval. Consequence to expect: the
		# spike-to-normal RATIO rises for enemies that now swing less often and falls
		...
		"loot": [["credits", 1200, 2500], ["Cu", 5, 12], ["Res2", 2, 4], ["CryoEssence", 3, 6], ["RimeplateScrap", 6, 12], ["Steel", 8, 18], ["Fe", 15, 35]],
		"rare_loot": [["AdvCircuit", 0.10, 1, 2], ["Ti", 0.15, 3, 8], ["Au", 0.20, 1, 3]],
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z4_shield", "z4_armor", "z4_battery", "z4_sensor", "z4_kinetic", "z4_energy", "z4_missile"],
		...
		"xp": 135, "zone": 4, "resist_k": -0.30, "resist_e": 0.00, "resist_x": 0.37, "dmg_type": "energy"
	},
	"z4_boss_overseer": {
	...
	"z5_xenon_scout": {
		"name": "Xenon Scout",
		"stats": {"hp": 5300, "max_shield": 1500, "atk": 366.666667, "def": 55, "atk_interval": 2.0, "accuracy": 65},
		...
		"xp": 300, "zone": 5, "resist_k": -0.30, "resist_e": 0.00, "resist_x": 0.37, "dmg_type": "energy"
	},
	"z5_xenon_corvette": {
		"name": "Xenon Corvette",
		...
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z5_kinetic", "z5_energy", "z5_missile", "z5_shield", "z5_armor", "z5_battery", "z5_sensor"],
		"xp": 350, "zone": 5, "resist_k": 0.37, "resist_e": -0.30, "resist_x": 0.00, "dmg_type": "energy"
	},
	"z5_alien_frigate": {
		"name": "Alien Frigate",
		...
		"loot": [["VoidArtifact", 2, 5], ["credits", 5000, 10000], ["Res2", 3, 6], ["XenoFragment", 2, 4], ["Circuit", 5, 10]],
		"rare_loot": [["QuantumCore", 0.08, 1, 1], ["AdvCircuit", 0.10, 2, 4]],
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z5_kinetic", "z5_energy", "z5_missile", "z5_battery", "z5_sensor", "z5_shield", "z5_armor"],
		"xp": 380, "zone": 5, "resist_k": 0.00, "resist_e": 0.37, "resist_x": -0.30, "dmg_type": "kinetic"
	},
	"z5_boss_harbinger": {
	...
	"z6_defense_turret": {
		"name": "Defense Turret",
		"stats": {"hp": 12700, "atk": 490.4, "def": 125, "atk_interval": 2.0, "accuracy": 90},
		...
		"xp": 700, "zone": 6, "resist_k": 0.37, "resist_e": 0.00, "resist_x": -0.30, "dmg_type": "kinetic"
	},
	"z6_mining_golem": {
		"name": "Mining Golem",
		"stats": {"hp": 15926, "atk": 441.714286, "def": 155, "atk_interval": 2.0, "accuracy": 85},
		"loot": [["Fe", 50, 100], ["Ti", 10, 25], ["Res3", 1, 2]],
		"rare_loot": [["Superalloy", 0.10, 2, 5]],
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z6_kinetic", "z6_energy", "z6_missile", "z6_shield", "z6_armor", "z6_battery", "z6_sensor"],
		"xp": 750, "zone": 6, "resist_k": -0.30, "resist_e": 0.37, "resist_x": 0.00, "dmg_type": "explosive"
	},
	"z6_ore_guardian": {
		"name": "Ore Guardian",
		...
		"loot": [["Fe", 30, 70], ["Steel", 10, 25], ["Res3", 1, 3], ["ColonySalvage", 4, 10], ["RadIsotope", 1, 3], ["U", 5, 12]],
		"rare_loot": [["VoidArtifact", 0.10, 1, 3], ["Ir", 0.08, 1, 2]],
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z6_shield", "z6_armor", "z6_battery", "z6_sensor", "z6_kinetic", "z6_energy", "z6_missile"],
		"xp": 720, "zone": 6, "resist_k": 0.00, "resist_e": -0.30, "resist_x": 0.37, "dmg_type": "explosive"
	},
	"z6_boss_colossus": {
	...
	"z7_shard_swarm": {
		"name": "Shard Swarm",
		# v142 idle-rule fix: dps cut ~27% (1350 -> 980 at the then-0.8s cadence) —
		# every config below Unique DIED on it, tier-matched Common included. v149
		...
		"xp": 1500, "zone": 7, "resist_k": 0.37, "resist_e": -0.30, "resist_x": 0.00, "dmg_type": "energy"
	},
	"z7_energy_wraith": {
		"name": "Energy Wraith",
		...
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z7_kinetic", "z7_energy", "z7_missile", "z7_shield", "z7_armor", "z7_battery", "z7_sensor"],
		"xp": 1800, "zone": 7, "resist_k": 0.00, "resist_e": 0.37, "resist_x": -0.30, "dmg_type": "energy"
	},
	"z7_gamma_beast": {
		"name": "Gamma Beast",
	...
	"z8_prism_drone": {
		"name": "Prism Drone",
		"stats": {"hp": 73000, "max_shield": 25000, "atk": 4000, "def": 600, "atk_interval": 2.0, "accuracy": 120},
		...
		"xp": 4000, "zone": 8, "resist_k": -0.30, "resist_e": 0.00, "resist_x": 0.37, "dmg_type": "energy"
	},
	"z8_crystal_golem": {
		"name": "Crystal Golem",
		...
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z8_kinetic", "z8_energy", "z8_missile", "z8_shield", "z8_armor", "z8_battery", "z8_sensor"],
		"xp": 4500, "zone": 8, "resist_k": 0.37, "resist_e": -0.30, "resist_x": 0.00, "dmg_type": "kinetic"
	},
	"z8_nebula_phantom": {
		"name": "Nebula Phantom",
	...
	"z9_plague_drone": {
		"name": "Plague Drone",
		"stats": {"hp": 175000, "max_shield": 50000, "atk": 10833.333333, "def": 1300, "atk_interval": 2.0, "accuracy": 140},
		...
		"xp": 10000, "zone": 9, "resist_k": 0.37, "resist_e": 0.00, "resist_x": -0.30, "dmg_type": "explosive"
	},
	"z9_bio_horror": {
		"name": "Bio-Horror",
		"stats": {"hp": 220160, "max_shield": 70000, "atk": 6584, "def": 1648, "atk_interval": 2.0, "accuracy": 145},
		"loot": [["BiohazardSample", 3, 8], ["Neutronium", 1, 2], ["Res3", 5, 10]],
		"rare_loot": [["PathogenCore", 0.08, 1, 1]],
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z9_kinetic", "z9_energy", "z9_missile", "z9_shield", "z9_armor", "z9_battery", "z9_sensor"],
		"xp": 12000, "zone": 9, "resist_k": -0.30, "resist_e": 0.37, "resist_x": 0.00, "dmg_type": "explosive"
	},
	"z9_quarantine_mech": {
		...
		"loot": [["Neutronium", 1, 3], ["credits", 500000, 1000000], ["Res3", 5, 10], ["Chip", 10, 25], ["AdvCircuit", 5, 12]],
		"rare_loot": [["PathogenCore", 0.10, 1, 2], ["ChronoCore", 0.05, 1, 1]],
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z9_shield", "z9_armor", "z9_battery", "z9_sensor", "z9_kinetic", "z9_energy", "z9_missile"],
		"xp": 11500, "zone": 9, "resist_k": 0.00, "resist_e": -0.30, "resist_x": 0.37, "dmg_type": "explosive"
	},
	...
	"z10_void_stalker": {
		...
		"xp": 25000, "zone": 10, "resist_k": 0.37, "resist_e": -0.30, "resist_x": 0.00, "dmg_type": "kinetic"
	},
	"z10_temporal_phantom": {
		"name": "Temporal Phantom",
		"stats": {"hp": 528384, "max_shield": 200000, "atk": 24140, "def": 3627, "atk_interval": 2.0, "accuracy": 170},
		...
		"module_drop_chance": 0.10,
		"module_drop_pool": ["z10_kinetic", "z10_energy", "z10_missile", "z10_shield", "z10_armor", "z10_battery", "z10_sensor"],
		"xp": 30000, "zone": 10, "resist_k": 0.00, "resist_e": 0.37, "resist_x": -0.30, "dmg_type": "energy"
	},
	...
	"z10_boss_leviathan": {
		...
		# v149: 3.75 -> 5.625. The death burst is atk x mult and fires per KILL, so its
		# rate is (kills/sec x magnitude). Kill rate is unchanged by the cadence
	...
	"z11_warp_revenant": {
		...
		"xp": 60000, "zone": 11, "warp_hardened": true, "resist_k": 0.0, "resist_e": 0.0, "resist_x": 0.0, "resist_cryo": -0.25, "dmg_type": "energy"
	},
	"z11_phase_horror": {
		"name": "Phase Horror",
		"stats": {"hp": 2000000, "max_shield": 300000, "atk": 56000, "def": 8500, "atk_interval": 2.0, "accuracy": 190},
		"loot": [["ChronoCore", 2, 4], ["VoidEssence", 3, 6]],
		"rare_loot": [["OmegaPlating", 0.08, 1, 2]],
		"module_drop_chance": 0.15,
		"module_drop_pool": ["cryo_lance"],
		"xp": 70000, "zone": 11, "warp_hardened": true, "resist_k": 0.0, "resist_e": 0.0, "resist_x": 0.0, "resist_cryo": -0.25, "dmg_type": "explosive"
	},
	"z11_null_sentinel": {
		"name": "Null Sentinel",
		"stats": {"hp": 2600000, "max_shield": 280000, "atk": 30400, "def": 10000, "atk_interval": 2.0, "accuracy": 185},
		"loot": [["OmegaPlating", 2, 4], ["PrimordialShard", 2, 4]],
		"rare_loot": [["VoidEssence", 0.10, 2, 4]],
		"module_drop_chance": 0.15,
		"module_drop_pool": ["cryo_lance"],
		"xp": 75000, "zone": 11, "warp_hardened": true, "resist_k": 0.0, "resist_e": 0.0, "resist_x": 0.0, "resist_cryo": -0.25, "dmg_type": "kinetic"
	},
	"z11_exotic_leviathan": {
		...
		"module_drop_chance": 0.15,
		"module_drop_pool": ["cryo_lance"],
		"xp": 80000, "zone": 11, "warp_hardened": true, "resist_k": 0.0, "resist_e": 0.0, "resist_x": 0.0, "resist_cryo": -0.25, "dmg_type": "kinetic"
	},
	"z11_boss_threshold_warden": {
		...
		"module_drop_chance": 0.30,
		"module_drop_pool": ["cryo_lance"],
		"is_boss": true, "xp": 1000000, "zone": 11, "warp_hardened": true, "resist_k": 0.0, "resist_e": 0.0, "resist_x": 0.0, "resist_cryo": -0.25, "dmg_type": "energy"
	},
	...
	"z12_acid_revenant": {
		...
		"xp": 120000, "zone": 12, "resist_k": 0.37, "resist_e": 0.00, "resist_x": -0.30, "resist_cryo": 0.0, "dmg_type": "energy"
	},
	"z12_rust_horror": {
		"name": "Rust Horror",
		"stats": {"hp": 2400000, "max_shield": 350000, "atk": 133333.333333, "def": 15000, "atk_interval": 2.0, "accuracy": 290},
		"loot": [["ChronoCore", 3, 6], ["VoidEssence", 4, 8]],
		"rare_loot": [["OmegaPlating", 0.10, 1, 3]],
		"module_drop_chance": 0.12,
		"module_drop_pool": ["corrosion_blaster"],
		"xp": 135000, "zone": 12, "resist_k": -0.30, "resist_e": 0.37, "resist_x": 0.00, "resist_cryo": 0.0, "dmg_type": "explosive"
	},
	...
	"z13_blight_drone": {
		...
		"xp": 180000, "zone": 13, "resist_k": 0.37, "resist_e": -0.30, "resist_x": 0.00, "resist_cryo": 0.0, "dmg_type": "energy"
	},
	"z13_corroded_golem": {
		"name": "Corroded Golem",
		"stats": {"hp": 4000000, "max_shield": 450000, "atk": 193333.333333, "def": 25000, "atk_interval": 2.0, "accuracy": 305},
		"loot": [["ChronoCore", 4, 8], ["VoidEssence", 5, 10]],
		"rare_loot": [["OmegaPlating", 0.10, 1, 3]],
		"module_drop_chance": 0.12, "module_drop_pool": ["corrosion_blaster"],
		"xp": 200000, "zone": 13, "resist_k": 0.00, "resist_e": 0.37, "resist_x": -0.30, "resist_cryo": 0.0, "dmg_type": "explosive"
	},
	"z13_patina_phantom": {
		"name": "Patina Phantom",
	...
	"z13_boss_verdigris_warden": {
		...
		"stats": {"hp": 65000000, "max_shield": 1000000, "atk": 240000, "def": 90000, "atk_interval": 2.0, "accuracy": 305},
	...
	"z14_dissolution_wraith": {
		...
		"rare_loot": [["VoidEssence", 0.10, 2, 4]],
		"module_drop_chance": 0.12, "module_drop_pool": ["corrosion_blaster"],
		"xp": 280000, "zone": 14, "resist_k": -0.30, "resist_e": 0.00, "resist_x": 0.37, "resist_cryo": 0.0, "dmg_type": "explosive"
	},
	"z14_caustic_golem": {
		"name": "Caustic Golem",
		"stats": {"hp": 7000000, "max_shield": 600000, "atk": 226666.666667, "def": 35000, "atk_interval": 2.0, "accuracy": 315},
		"loot": [["ChronoCore", 6, 12], ["OmegaPlating", 5, 10]],
		"rare_loot": [["PrimordialShard", 0.12, 4, 8]],
		"module_drop_chance": 0.12, "module_drop_pool": ["corrosion_blaster"],
		"xp": 305000, "zone": 14, "resist_k": 0.37, "resist_e": -0.30, "resist_x": 0.00, "resist_cryo": 0.0, "dmg_type": "kinetic"
	},
	"z14_toxin_sentinel": {
		"name": "Toxin Sentinel",
	...
	"z15_caustic_revenant": {
		...
		"module_drop_chance": 0.12, "module_drop_pool": ["corrosion_blaster"],
		"xp": 400000, "zone": 15, "resist_k": 0.37, "resist_e": 0.00, "resist_x": -0.30, "resist_cryo": 0.0, "dmg_type": "kinetic"
	},
	"z15_meltdown_colossus": {
		"name": "Meltdown Colossus",
		"stats": {"hp": 12000000, "max_shield": 750000, "atk": 266666.666667, "def": 50000, "atk_interval": 2.0, "accuracy": 325},
		"loot": [["ChronoCore", 8, 16], ["OmegaPlating", 7, 14]],
		"rare_loot": [["PrimordialShard", 0.12, 5, 10]],
		"module_drop_chance": 0.12, "module_drop_pool": ["corrosion_blaster"],
		"xp": 430000, "zone": 15, "resist_k": -0.30, "resist_e": 0.37, "resist_x": 0.00, "resist_cryo": 0.0, "dmg_type": "energy"
	},
	"z15_blight_titan": {
		"name": "Blight Titan",
	...
	"hz_emp_elite": {
		"name": "EMP Commander",
		"stats": {"hp": 2500, "max_shield": 600, "atk": 80, "def": 40, "atk_interval": 2.0, "accuracy": 50},
...
# v142 TIER-STEP REBASE — the enemy half. Player gear steps 3.75x/zone now
# (shipyard_manager.TIER_STEP_NEW); enemies take the SAME factor so in-zone
# difficulty is unchanged and ONLY the cross-zone gear gap widens. Scales the
# authored stat literals (hand-tuned per-zone shape preserved). accuracy /
# atk_interval are ratios, not power — never scaled. (v145: the enemy `eva`
# key is vestigial dead data — the player-accuracy axis it fed is gone.)
...
func _apply_enemy_tier_rebase() -> void:
	...
	for eid in enemy_db:
		var e: Dictionary = enemy_db[eid]
		var z: int = int(e.get("zone", 0))
		if z <= 1:
			continue
		var f: float = sm_script.tier_rebase(z)
...
func spawn_enemy():
	...
	current_enemy = {
		...
		# v145: enemy "eva" is GONE. It fed the player accuracy-vs-evasion hit
		# roll, which never worked — this read looked inside ["stats"] while all
		# 83 enemy defs declare eva TOP-LEVEL, so current_enemy.eva was always 0
		# and hit_chance was always 1.000. Removed rather than repaired: fixing it
		# would have been a 5-28% global DPS nerf that invalidates every table in
		# docs/design/ZONE_BASELINE_v142.md, to un-break a stat that was weak even
		# when working. NOTE: the enemy "accuracy" copied above is a DIFFERENT,
		# LIVE axis (it feeds the player's dodge roll in do_enemy_attack) — keep it.
		# v62.0 Fix: Copy atk_interval so enemy attack speed is used
		"atk_interval": e_data["stats"].get("atk_interval", DEFAULT_ATTACK_INTERVAL),
	...
	_reset_trait_state()  # v139d: P3 boss-trait per-fight state
	# v139d: copy P3 trait fields into the live enemy (the resist_cryo lesson —
	# anything the fight logic reads MUST be on current_enemy, not just the def).
	for _tk in ["sustain", "reactive_armor", "adaptive_grid", "charge_nuke", "volatile", "corrosive_field"]:
		if e_data.has(_tk):
			current_enemy[_tk] = e_data[_tk]
	...
	if _ezone >= 3 and not e_data.get("warp_hardened", false) and (e_data.get("phases", []) as Array).is_empty():
		...
		# v142 IDLE RULE (owner, 2026-07-25): "common new-tier gear must farm e3/e4
		# with consumable support WITHOUT DYING — if it dies there is no point of
		# idle." A clean tier-matched Common set is the INTENDED answer for its own
		# zone, so it must hold the line unattended. _zatk was killing it (measured
		# deaths at Z3 e3, Z5 e4, Z9 e3); _zhp was stretching kills past a farmable
		# rate. Both eased — steepening now shapes the curve, it does not gate.
		var _zhp: float = min(1.60, 1.20 + 0.08 * float(_ezone - 3))  # z3 ≈1.20× … z8+ ≈1.60×
		var _zdef: float = 1.5                                        # 0.80 mitig clamp keeps it killable
		var _zatk: float = min(1.40, 1.15 + 0.05 * float(_ezone - 3)) # z3 ≈1.15× … z8+ ≈1.40×
...
func retreat():
	...
	# v143: nudge the mission poll. mission_manager has no tick — sync_progress runs
	# only at setup, on shipyard updates and when a mission starts — so the
	# "defeat_retreat" state check would otherwise fire late or not at all.
	if GameState.mission_manager:
		GameState.mission_manager.sync_progress()
...
func _apply_trinity_stat_bonuses(sm):
	...
	# v145: the accuracy_flat set line is gone with the whole player-accuracy axis.
	# Overseer's Command now grants enemy_drop_mult, which is read directly at the
	# win_fight loot site — NOT written into sm here, because this function is
	# called per fight-start and only recalc_stats() clears sm.affix_bonuses, so an
	# additive write would compound every spawn.
...
func process_tick(delta: float):
	...
	var e_interval = current_enemy.get("atk_interval", DEFAULT_ATTACK_INTERVAL)
	...
	_trait_tick(delta)  # v139d P3: sustain pulse/nanite + corrosive field
...
func _execute_player_attack(weapon_idx: int):
	...
	# v145: PLAYER ACCURACY vs ENEMY EVASION IS GONE. The roll here was
	#   hit_chance = clamp(p_acc / (p_acc + e_eva), 0.2, 1.0)
	# and e_eva came from current_enemy["eva"], which spawn_enemy read out of
	# e_data["stats"] while every enemy def declares eva TOP-LEVEL. e_eva was
	# therefore always 0 and hit_chance always 1.000 — the player has never
	# missed. Repairing it would have cost a 5-28% global DPS nerf and a full
	# recalibration sweep, for a stat that moved DPS ~1.7% per sensor even when
	# working. Deleted instead; sensors got a real identity in the same pass
	# (shipyard_manager SENSOR SUITES). The player's own EVASION vs the enemy's
	# accuracy is a SEPARATE, working axis and stays — see do_enemy_attack.
	...
	enemy_hp -= res[1]
	_trait_note_player_hit(String(w.get("type", "kinetic")))  # v139d: Reactive/Adaptive growth
...
# ─────────────────────────────────────────────────────────────────────────────
# P3 Boss-trait engine (v139d) — data-driven encounter mechanics
# (docs/design/P3_BOSS_MECHANICS.md). Fields live on the enemy def and are
# copied at spawn; every trait resolves in math (pre-fight solvable, no new
# in-fight inputs). NO enemy carries trait data until the Rare-gate retune
# assigns it, so the engine ships balance-neutral.
# ─────────────────────────────────────────────────────────────────────────────
func _reset_trait_state() -> void:
	_ra_steps = 0
	_ra_hits = 0
	_ag_added = {"kinetic": 0.0, "energy": 0.0, "explosive": 0.0, "cryo": 0.0}
	_cn_count = 0
	_cn_is_nuke = false
	_sus_timer = 0.0
	_nanite_left = 0.0
...
func _trait_note_player_hit(w_type: String) -> void:
	if current_enemy == null:
		return
	var ra: Dictionary = current_enemy.get("reactive_armor", {})
	if not ra.is_empty():
		_ra_hits += 1
		if _ra_hits >= int(ra.get("per_hits", 25)):
			_ra_hits = 0
	...
	if not ag.is_empty():
		...
		if ch == "missile":
			ch = "explosive"
		if not _ag_added.has(ch):
			ch = "kinetic"
		var cap := float(ag.get("cap", 0.5))
		var step := float(ag.get("per_hit_resist", 0.02))
		if float(_ag_added[ch]) < cap:
			_ag_added[ch] = min(cap, float(_ag_added[ch]) + step)
			# Mutate the live enemy's resist — resolve_damage reads it every hit,
			# so the growth lands with zero extra plumbing.
			var rk := "resist_cryo"
			match ch:
				"kinetic": rk = "resist_k"
				"energy": rk = "resist_e"
...
func _trait_tick(delta: float) -> void:
	if current_enemy == null or not in_combat:
		return
	...
	if not sus.is_empty():
		match String(sus.get("kind", "")):
			"pulse":
				_sus_timer += delta
				var every: float = max(1.0, float(sus.get("every_s", 8.0)))
				if _sus_timer >= every:
					_sus_timer -= every
			...
			"nanite":
				...
				if _nanite_left > 0.0:
					_nanite_left -= delta
					enemy_hp = min(enemy_max_hp, enemy_hp + enemy_max_hp * float(sus.get("hull_pct_per_s", 0.04)) * delta)
	var cf: Dictionary = current_enemy.get("corrosive_field", {})
	if not cf.is_empty():
		var sm_cf = GameState.shipyard_manager
...
func _execute_enemy_attack():
	...
	# v139d P3 trait: Charge Nuke — count every swing; the Nth is the nuke, and
	# the swing before it telegraphs (active players may pop a shield consumable
	# on the telegraph — an EXISTING button, so the auto-battler contract holds).
	_cn_is_nuke = false
	var _cn: Dictionary = current_enemy.get("charge_nuke", {}) if current_enemy else {}
	if not _cn.is_empty():
		_cn_count += 1
		var _cn_n: int = max(2, int(_cn.get("every_n", 5)))
		if _cn_count >= _cn_n:
			_cn_count = 0
			_cn_is_nuke = true
	...
	if randf() < dodge_chance:
		...
		# shield and feeds the boss's own (min-DPS check: out-damage the theft).
		var _sus_s: Dictionary = current_enemy.get("sustain", {})
		if String(_sus_s.get("kind", "")) == "siphon":
			var _steal: float = min(player_shield, player_max_shield * float(_sus_s.get("pct", 0.04)))
			if _steal > 0.0:
...
func get_effective_module_drop_chance(enemy_data: Dictionary) -> float:
	...
	# (a deliberate +rare-loot investment). v145: accuracy no longer exists at all.
	var rm = GameState.research_manager
	var xeno_bonus = rm.get_efficiency_bonus("xeno_engineering") if rm else 0.0
	# v128: Sensor "Salvage Scanner" affix (module_drop_mult).
	# v145: the CRAFTED sensor line now carries a base module_drop_mult too (it
	# replaced the dead `accuracy` stat), and the Topaz utility facet grants it in
	# place of the dead accuracy_flat. All three stack additively, then multiply.
	var sm = GameState.shipyard_manager
	var salvage = 0.0
	if sm:
		salvage = sm.affix_bonuses.get("module_drop_mult", 0.0) + sm.gem_bonuses.get("module_drop_mult", 0.0)
...
func win_fight():
	...
	if not _vol.is_empty() and not _volatile_fired:
		...
		var _sm_v = GameState.shipyard_manager
		var _burst: float = float(current_enemy.get("atk", 0)) * float(_vol.get("mult", 3.0))
	...
	# v145: the CRAFTED sensor line contributes a base enemy_drop_mult (aggregated
	# into affix_bonuses by recalc_stats), and Overseer's Command adds a flat 25%.
	# The set value is read HERE rather than written into sm, so it can't compound
	# across fight-starts (see _apply_trinity_stat_bonuses).
	var sm_loot = GameState.shipyard_manager
	if sm_loot:
		loot_mult *= (1.0 + sm_loot.affix_bonuses.get("enemy_drop_mult", 0.0)
			+ _get_set_bonus_value("enemy_drop_mult") / 100.0)
...
func _offline_winnable() -> bool:
	...
	var cryo_dps := 0.0
	for w in player_weapon_states:
		var iv: float = max(MIN_ATTACK_INTERVAL, float(w.get("interval", DEFAULT_ATTACK_INTERVAL)))
	...
	match String(_sus_o.get("kind", "")):
		"pulse":
			...
			dps -= _regen * 2.0
		"siphon":
			var _theft: float = GameState.shipyard_manager.max_shield * float(_sus_o.get("pct", 0.04)) / max(0.5, float(current_enemy.get("atk_interval", DEFAULT_ATTACK_INTERVAL)))
			dps -= _theft * 2.0
		"nanite":
