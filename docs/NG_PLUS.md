...
**P2 now complete (gameplay):** the **Threshold Relic** master key — dedicated `equipped_relic` slot (separate from the hull grid), `rift_relic` drop (guaranteed + auto-equipped on the Rift Warden's first clear), an incoming-damage cut to ~8% **only in its keyed zone** (`get_relic_reduction_factor`, applied in `_execute_enemy_attack`), cross-warp persistence via a `<id>_earned` flag (re-granted in `execute_warp`; cleared on hard reset), and save/load. So the full Z12 loop — unlock → clear (Cryo→Corrosion) → relic → idle-farm — is playable and sim-verified.

**Remaining:** P2 UI — the combat-page swap-preset buttons + a relic-management panel (both UI, not headless-verifiable). NOTE: the swap is only strictly *needed* at **3+ phase bosses** (WT3+); a 2-phase boss like the Rift Warden is already beatable with a mixed Cryo+Corrosion loadout (each phase, the matching weapon does full, the other chips ×0.15). P3 — `corrosion_plating` resist module + acid-DoT enemy attack (the defensive axis) + per-element weapon UI coloring; then WT3–5 (Thermal/Radiation/Graviton). P4 — Fleet siege gates. P5 — sim-tune the HP/cut/scaling curves.
