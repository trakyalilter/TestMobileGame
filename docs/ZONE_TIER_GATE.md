...
**Status:** Spec LOCKED. **ENGINE SLICE BUILT & sim-verified** (2026-06-18) — `phase_gate_spike.gd` (+20 tier-gate asserts, ALL PASS) + full headless boot clean (exit 0): the `tier_hardened` floor (per-weapon offense + defense armor/shield factor), the position-derived front/back split (`spawn_enemy`), module-loot routing, and the new-game-only `tier_gate_enabled` flag with the old-save ungate migration. Combat-log telegraphs ("HARDENED HULL" / "ARMOR OUTCLASSED") are in. **REMAINING = the content/demand layer** (signature raws + alloys + refine recipes + re-pointed common recipes per the locked table), the pre-fight **e3/e4 card badge** (UI), and the z3/z4 "Cryo" naming cleanup. All design decisions below are settled; numbers marked *(tune)* are first-pass values for the sim/playtest pass, never the design itself.
